...
function getFallbackRecipes() {
    return [
        { id: 1, name: 'عدسی', category: 'خورش', ingredients: [{ name: 'عدس', quantity: 0.05, unit: 'کیلوگرم' }, { name: 'برنج', quantity: 0.1, unit: 'کیلوگرم' }, { name: 'پیاز', quantity: 0.02, unit: 'کیلوگرم' }, { name: 'روغن', quantity: 0.02, unit: 'لیتر' }], servings: 4, cook_time: 45, difficulty: 'آسان', nutrition: { calories: 180, protein: 8, carbs: 28, fat: 2, fiber: 5 }, tags: ['گیاهی', 'ارزان', 'سریع'], season: 'همه فصول' },
        { id: 2, name: 'ماکارونی با رب', category: 'پاستا', ingredients: [{ name: 'ماکارونی', quantity: 0.15, unit: 'کیلوگرم' }, { name: 'رب گوجه', quantity: 0.01, unit: 'کیلوگرم' }, { name: 'روغن', quantity: 0.02, unit: 'لیتر' }], servings: 4, cook_time: 30, difficulty: 'آسان', nutrition: { calories: 250, protein: 8, carbs: 40, fat: 3, fiber: 3 }, tags: ['سریع', 'ارزان'], season: 'همه فصول' },
        { id: 3, name: 'املت ساده', category: 'صبحانه', ingredients: [{ name: 'تخم‌مرغ', quantity: 2, unit: 'عدد' }, { name: 'روغن', quantity: 0.02, unit: 'لیتر' }, { name: 'نمک', quantity: 0.002, unit: 'کیلوگرم' }], servings: 2, cook_time: 10, difficulty: 'آسان', nutrition: { calories: 180, protein: 12, carbs: 1, fat: 14, fiber: 0 }, tags: ['سریع', 'صبحانه'], season: 'همه فصول' }
    ];
}
...
export async function generateWeeklyPlan(days = 7, startDate = null) {
    ...
    const recipeAvailability = recipes.map(recipe => ({ ...recipe, servings: calculateServings(recipe, inventory, familySize), isAvailable: calculateServings(recipe, inventory, familySize) > 0 }));
    ...
    const expiringItems = inventory.filter(item => { if (!item.expiry) return false; const daysLeft = (new Date(item.expiry) - new Date()) / (1000 * 60 * 60 * 24); return daysLeft >= 0 && daysLeft <= 3; });
    ...
    for (let i = 0; i < maxDays; i++) {
        ...
        const dayPlan = { day: i + 1, date: date.toISOString().slice(0, 10), dayName: dayName, meals: {} };
        mealTypes.forEach(type => {
            const recipe = selectMeal(type, i, usedMeals);
            dayPlan.meals[type] = { name: recipe.name, ingredients: recipe.ingredients, cook_time: recipe.cook_time, servings: recipe.servings };
...
export async function generateConsumptionPlan(days = 7, startDate = null) {
    ...
    let html = `<div class="consumption-plan"><div class="flex justify-between items-center mb-4"><h4 class="text-lg font-bold text-primary">📅 برنامه مصرف (${maxDays} روز)</h4><span class="text-sm text-gray-500">${crisisMode ? '⚠️ بحران' : '🌿 عادی'}</span></div><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">`;
    plan.forEach((day, idx) => {
        ...
        ['صبحانه', 'ناهار', 'شام'].forEach(type => {
            ...
            const icon = type === 'صبحانه' ? '🌅' : type === 'ناهار' ? '🌞' : '🌙';
            html += `<div class="meal-item cursor-pointer hover:bg-blue-50 p-1 rounded transition-colors" data-day-index="${idx}" data-meal-type="${type}"><span class="font-medium">${icon} ${type}:</span> ${meal.name} <span class="text-xs text-gray-400">(⏱️ ${meal.cook_time || '?'} دقیقه)</span></div>`;
        });
        html += `</div></div>`;
    });
    html += `</div><div class="mt-4 p-3 bg-gray-50 rounded-xl"><h5 class="text-sm font-bold text-gray-700 mb-2">📊 خلاصه ذخایر کلیدی</h5><div class="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">`;
    if (keyItems.water) html += `<div>💧 آب: ${keyItems.water.quantity} ${keyItems.water.unit}</div>`;
    if (keyItems.rice) html += `<div>🍚 برنج: ${keyItems.rice.quantity} ${keyItems.rice.unit}</div>`;
    if (keyItems.legumes) html += `<div>🫘 حبوبات: ${keyItems.legumes.quantity} ${keyItems.legumes.unit}</div>`;
    if (keyItems.canned) html += `<div>🥫 کنسرو: ${keyItems.canned.quantity} ${keyItems.canned.unit}</div>`;
    html += `</div></div><div class="mt-3 p-3 bg-blue-50 rounded-xl border border-blue-200"><h5 class="text-sm font-bold text-blue-700 mb-1">💡 نکات هوشمند</h5><ul class="text-xs text-blue-600 space-y-1">`;
    tips.forEach(t => html += `<li>${t}</li>`);
    html += `</ul></div>`;
    if (crisisMode) {
        html += `<div class="mt-3 p-3 bg-red-50 rounded-xl border border-red-200"><p class="text-sm text-red-700">⚠️ حالت بحران: اولویت با آب، کنسرو و مواد خشک است.</p></div>`;
    }
    html += `</div>`;
    return html;
}
...
export function getMealDetails(dayIndex, mealType, plan) {
    ...
    if (!meal) return null;
    return { dayName: day.dayName, date: day.date, mealType, mealName: meal.name, ingredients: meal.ingredients, cook_time: meal.cook_time, servings: meal.servings, dayIndex };
}

export function getConsumptionPlanOptions() {
    return { daysOptions: [3, 5, 7, 14, 30], defaultDays: 7 };
}

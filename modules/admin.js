// modules/admin.js
// پنل مدیریت سازمانی - تمام منطق اصلی

import { getAdminSession, adminLogout, getCurrentOrganization } from './admin-auth.js';
import { store } from './store.js';

// ============================================================
// کلیدهای ذخیره‌سازی داده‌ها
// ============================================================
const CONSUMPTION_KEY = 'admin_consumption_data';
const USERS_KEY = 'app_users';
const ADMIN_REPORTS_KEY = 'admin_reports';

// ============================================================
// بارگذاری داده‌های مصرف از localStorage (همه کاربران)
// ============================================================
function loadAllConsumptionData() {
    const allData = {};
    try {
...
function calculateStatistics(consumptionData) {
    ...
    for (const [username, data] of Object.entries(consumptionData)) {
        if (data && data.dates && data.dates.length > 0) {
            ...
            // میانگین روزانه هر کاربر
            const days = data.dates.length;
            totalDays += days;
            
            // جمع‌آوری داده‌های روزانه برای نمودار
...
export function renderAdminDashboard() {
    ...
    // نمایش نام سازمان
    document.getElementById('adminOrgName').textContent = session.organizationName || 'سازمان';
    
    // بارگذاری داده‌ها
    const consumptionData = loadAllConsumptionData();
    const users = getAllUsers();
    const stats = calculateStatistics(consumptionData);
    
    // رندر کارت‌های آماری
    renderStatsCards(stats, users);
    
    // رندر نمودارها
    renderCharts(stats);
    
    // رندر جدول کاربران
    renderUsersTable(users, consumptionData);
    
    // رندر هشدارهای مصرف
    renderAlerts(users, consumptionData);
    
    // رندر پیش‌بینی مصرف
    renderForecast(stats);
}

// ============================================================
// رندر کارت‌های آماری
// ============================================================
function renderStatsCards(stats, users) {
    const cards = [
        { label: 'تعداد کاربران', value: stats.totalUsers, icon: 'fa-users', color: 'blue' },
        { label: 'کل مصرف آب (لیتر)', value: stats.totalWater.toLocaleString(), icon: 'fa-water', color: 'blue' },
        { label: 'کل مصرف برق (کیلووات)', value: stats.totalElectricity.toLocaleString(), icon: 'fa-bolt', color: 'yellow' },
        { label: 'کل مصرف گاز (مترمکعب)', value: stats.totalGas.toLocaleString(), icon: 'fa-fire', color: 'orange' },
...
// رندر جدول کاربران
// ============================================================
function renderUsersTable(users, consumptionData) {
    ...
    users.forEach(user => {
        ...
        tr.innerHTML = `
            ...
            <td class="px-4 py-3">
                <button onclick="window.viewUserDetails('${user.username}')" class="text-blue-500 hover:text-blue-700 text-sm">
    ...
    modal.innerHTML = `
        <div class="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl">
            <div class="flex justify-between items-center mb-4">
                ...
                <button onclick="this.closest('#userDetailModal').remove()" class="text-gray-400 hover:text-gray-600">
            ...
            <div class="space-y-3">
                ...
                ${data.dates.length > 0 ? `
                    <div class="mt-3">
                        ...
                        <canvas id="userDetailChart" height="150"></canvas>
                    </div>
                ` : `
...
// رندر هشدارهای مصرف
// ============================================================
function renderAlerts(users, consumptionData) {
...
export function initAdminDashboard() {
    ...
    document.getElementById('exportCSVBtn')?.addEventListener('click', window.exportCSV);
